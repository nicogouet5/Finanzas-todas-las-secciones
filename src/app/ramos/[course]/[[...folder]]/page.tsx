...
export default async function CoursePage({ params }: { params: Promise<{ course: string; folder?: string[] }> }) {
  ...
  return <main id="main" className="page"><Breadcrumbs items={crumbs} /><h1>{folder.at(-1)?.replaceAll("-", " ") ?? courseLabel(course)}</h1>{!current.folders.length && !current.files.length ? <p className="empty-state">Aún no hay materiales en este ramo.</p> : <><section className="folder-grid" aria-label="Carpetas">{current.folders.map((item) => <Link className="folder-card" key={item.path} href={`/ramos/${course}/${item.path.split("/").slice(1).join("/")}`}><span>Carpeta</span><h2>{item.name.replaceAll("-", " ")}</h2></Link>)}</section><section className="material-grid" aria-label="Materiales">{current.files.map((item) => <MaterialCard item={item} key={item.path} />)}</section></>}</main>;
}
